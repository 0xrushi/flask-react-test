...
import OneCrossThree from './components/one-cross-three';
import OneFullWidth from './components/one-full-width';
import './App.css';
import MarketDirectionCards from './components/market-direction-cards';
import NegativeValuesBarChart from './components/negative-values-bar-chart';
import ChartCard from './components/chart-card';
import OneCrossThree2 from './components/one-cross-three2';
...
function App() {
  ...
  useEffect(() => {
    fetch('/getstock?symbol=qqq&interval=15').then(res => res.json()).then(data => {
      ...
      // console.log(JSON.stringify(data["Time Series (15min)"]));
      // var i="", j="";
      // let tmp = JSON.parse(state['root']);
      // for (i in tmp){
      //   tmp[i]["x"] = Date.parse(tmp[i]["x"]);
      //   tmp[i]["y"] = tmp[i]["y"].map(Number);
      // }

      // console.log(state);

    // state['data']=[];
    // for (i in JSON.parse(state['root'])){
    //     // console.log(JSON.stringify(JSON.parse(state['root'])[i]));
    //     // console.log(JSON.stringify(Object.values(JSON.parse(state['root'])[i]).slice(0, 4)));

    //     var temp = {
    //         'x': JSON.stringify(i),
    //         'y': JSON.stringify(Object.values(JSON.parse(state['root'])[i]).slice(0, 4))
    //     }

        // state['root']=(JSON.stringify(tmp));
    // }
    // console.log( data);
    ...
    // console.log( JSON.stringify(Object.values(state['data'])));
    });
    ...
    // var chart = new ApexCharts(document.querySelector("#chart2"), options).render();
    // var chart = new ApexCharts(document.querySelector("#chart3"), options).render();
    // var chart = new ApexCharts(document.querySelector("#chart4"), options).render();
